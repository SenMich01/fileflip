...
export default function FileUploader({ onConversionComplete }) {
  ...
  const validateFile = useCallback((f) => {
    ...
    const mime = f.type;
    const acceptedMimes = Array.isArray(conversion.acceptMime)
      ? conversion.acceptMime
      : [conversion.acceptMime];
    if (!acceptedMimes.includes(mime)) {
      return `Invalid file type. Expected: ${conversion.accept}`;
    }
    return null;
  }, [conversion]);
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      setSuccess(`✓ Converted successfully! "${downloadName}" is downloading.`);
      setFile(null);
  ...
  return (
    <div className="card p-6 sm:p-8">
      <h2 className="font-display text-xl font-bold text-white mb-6">Convert a File</h2>
      ...
      >
        ...
        {file ? (
          <div className="flex flex-col items-center gap-3">
            <div className="w-12 h-12 bg-electric-500/15 border border-electric-500/30 rounded-xl flex items-center justify-center">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); resetState(); if (inputRef.current) inputRef.current.value = ""; }}
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div>
              <p className="text-white/50 text-sm">
                <span className="text-electric-400 font-medium">Click to browse</span> or drag & drop
              </p>
